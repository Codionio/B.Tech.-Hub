/**
 * SGPA Calculator - Grade Viewer Version (Final Merged Code)
 *
 * This version implements in-table dropdowns for selectable first-year subjects,
 * enhances the final result display with a detailed breakdown table,
 * and maintains the real-time grade display next to each marks input.
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateSubjects() {
        ...
        if (year && semester) {
            if (year === 'First Year') {
                // For first year, we now pass all subjects directly, including those with '/' for selection.
                // The choice is no longer made by the 'Group' dropdown.
                subjects = subjectData[year]?.[semester] || {};
                emptyMessage = "No subjects found for first year.";
    ...
    function renderSubjectsTable(subjects, emptyMessage) {
        ...
        Object.entries(subjects).forEach(([subjectName, credits]) => {
            ...
            if (subjectName.includes(' / ')) {
                ...
                // Otherwise, just display the subject name as text.
                subjectCellHTML = `<span class="subject-name">${subjectName}</span>`;
            }
    ...
    function calculateSGPA() {
        ...
        rows.forEach(row => {
            ...
            const name = subjectSelect ? subjectSelect.value : (subjectSpan ? subjectSpan.textContent : 'Unknown Subject');
