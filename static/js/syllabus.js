document.addEventListener('DOMContentLoaded', function() {
    ...
    const yearSelectionTitle = document.getElementById('yearSelectionTitle');
    const syllabusTitle = document.getElementById('syllabusTitle');
    const syllabusIframe = document.getElementById('syllabusIframe');
    ...
    window.selectBranch = function(branchCode, branchName) {
        currentBranchCode = branchCode;
        currentBranchName = branchName.toUpperCase();
        branchSelection.classList.add('hidden');
        yearSelection.classList.remove('hidden');
        yearSelectionTitle.textContent = `Select Year - ${currentBranchName}`;
    };
    ...
    window.selectYear = function(year) {
        ...
        const pdfUrl = syllabusData[currentBranchCode]?.[year];
        if (pdfUrl) {
            syllabusTitle.textContent = `Syllabus - ${currentBranchName} Year ${year}`;
            syllabusIframe.src = pdfUrl;
        } else {
            syllabusTitle.textContent = "Syllabus not found";
            syllabusIframe.src = '';
        }
    };
    ...
    window.goBack = function(from) {
        if (from === 'branch') {
            ...
            yearSelection.classList.remove('hidden');
            syllabusIframe.src = ''; // Stop PDF from loading in background
        }
    };
});